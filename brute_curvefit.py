...
def bruteforce(func, x, y,  restrict, ntol = 1000, returnnfactor = 0.01, printerrors=True, parallel=False):
    '''
    ...
    if parallel:
        ...
        A = pool.map(funcnorm_par, zip(np.tile(func,ntol), args_list, np.tile(ymin,ntol), np.tile(yrange,ntol)))
        for a in A:
...
def scipy_fit(func, x, y,  restrict, p0list, maxfev = 1000, printerrors=True, parallel=False ):
    '''
    func is any function with the first arguemnet the list of Independent variable and all the next variables free parameters.
    x is the list of values of the Independent variable. y is the actual data to be fitted to.
    restrict is list of two lists of the form [[minA, minB, minC],[maxA, maxB, maxC]] where minI and maxI are the minimum and maximum value parameter I can take
    ...
    if parallel:
        ...
        pool = Pool(processes=os.cpu_count()-2)
        A = pool.map(scipy_fit_parhelper, zip([func]*len(p0list), [ymin]*len(p0list),[yrange]*len(p0list),[x]*len(p0list),[ynorm]*len(p0list),[restrict]*len(p0list),p0list,[maxfev]*len(p0list),[printerrors]*len(p0list) ))
        for a in A:
            fittedparam = a[0]
            error = a[1]
...
def brute_scifit(func, x, y,  restrict, ntol = 1000, returnnfactor = 0.01, maxfev = 1000, printerrors=True, parallel=False):
    '''
    func is any function with the first arguemnet the list of Independent variable and all the next variables free parameters.
    x is the list of values of the Independent variable. y is the actual data to be fitted to.
    restrict is list of two lists of the form [[minA, minB, minC],[maxA, maxB, maxC]] where minI and maxI are the minimum and maximum value parameter I can take
    ...
    returns the best model, and its error
    '''
    paramsfitted,errors = bruteforce(func,x,y, restrict= restrict,ntol=ntol,returnnfactor=returnnfactor, printerrors=printerrors, parallel=parallel)
    paramfitted,error = scipy_fit(func,x,y, restrict= restrict, p0list=paramsfitted, maxfev = maxfev, printerrors=printerrors, parallel=parallel)
    return [paramfitted,error]

def brute_then_scipy(func, x, y,  restrict, ntol = 1000, returnnfactor = 0.01, maxfev = 1000, printerrors=True, parallel=False):
    '''
    Alternate name for brute_scifit. Included for backward compatibility.
    '''
    return brute_scifit(func, x, y,  restrict, ntol = 1000, returnnfactor = 0.01, maxfev = 1000, printerrors=printerrors, parallel=parallel)
